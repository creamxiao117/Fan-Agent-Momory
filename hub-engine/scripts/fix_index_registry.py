...
## 边界（2026-09-23 裁定：与 fix_orphans 保持分开）
本工具只做「**INDEX 有 / 文件无**」这一个方向：幽灵 slug（登记名 ≠ 文件名）的纠偏。
因为它**会改名源文件**，属于较高危险级别，故与「纯追加登记行」的
`fix_orphans.py`（负责「文件有 / INDEX 无」）**有意分开**——
合并会让 `--apply` 的语义变模糊（会动源文件吗？）。
共享契约在 `scripts/index_consistency.py`（分区表 / 登记行解析 / slug 集合），
所以分开不会导致口径漂移。
