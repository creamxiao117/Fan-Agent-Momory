...
| 运行态数据中枢（Obsidian 库，唯一事实源） | `D:\AIwork\AgentMemoryHub` |
| 跨项目经验/规则（DLL 防锁、查询回写等） | `D:\AIwork\AgentMemoryHub\rules\experience` + INDEX.md |

## 执行前必读（用户既定规则）

- 先查统一记忆中枢：读 `D:\AIwork\AgentMemoryHub\INDEX.md` 与 `rules/`、`experience/`，命中再执行。
